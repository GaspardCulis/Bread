...
public:
     ...
     */
    vector<Vector3<int>> findBlocks(std::function<bool(const Blockstate *block, const Position position, std::shared_ptr<World> world)> match_function, const int search_radius = 64, const int max_results = -1, const std::optional<Position> origin = std::nullopt) const;
     ...
     */
    Vector3<int> findNearestBlock(std::function<bool(const Blockstate *block, const Position position, std::shared_ptr<World> world)> match_function, const int search_radius = 64, const std::optional<Position> origin = std::nullopt) const;
